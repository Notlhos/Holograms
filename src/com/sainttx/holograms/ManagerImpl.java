...
public class ManagerImpl implements HologramManager {
	...
	private final Map<String, Hologram> activeHolograms = new TreeMap<>(String.CASE_INSENSITIVE_ORDER);
	private final Set<UpdatingHologramLine> trackedUpdatingLines = new HashSet<>();
	...
	void load() {
		...
		if (persistingHolograms.isConfigurationSection("holograms")) {
			loadHolograms: for (String hologramName : persistingHolograms.getConfigurationSection("holograms").getKeys(false)) {
	...
	public void saveHologram(Hologram hologram) {
		...
		List<String> uncoloredLines = holoLines.stream().map(HologramLine::getRaw).collect(Collectors.toList());
