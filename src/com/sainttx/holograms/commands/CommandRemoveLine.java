			...
			if (hologram == null) {
				...
				if ((index < 0) || (index >= hologram.getLines().size())) {
					...
					if (line instanceof TextualHologramLine) {
						sender.sendMessage(TextUtil.color("&7Removed line &f\"" + ((TextualHologramLine) line).getText() + "&f\" &7from hologram &f\"" + hologram.getId() + "\"."));
					} else {
						sender.sendMessage(TextUtil.color("&7Removed line at position &f\"" + index + "&f\" &7from hologram &f\"" + hologram.getId() + "\"."));
					}
