...
public abstract class HologramPlugin extends JavaPlugin {
	...
	private final Set<HologramLine.Parser> parsers = new HashSet<>();

	/**
	 * Parses and returns a {@link HologramLine} from a textual representation. This method will return a {@link TextLine} if no valid parser is found.
	 *
	 * @param hologram
	 *            the parent hologram
	 * @param text
	...
	public HologramLine parseLine(Hologram hologram, String text) {
		Optional<HologramLine.Parser> parser = parsers.stream().filter(p -> p.canParse(text)).findFirst();
	...
	/**
	 * Returns the entity controller instance for the currently active version of CraftBukkit software.
	 *
